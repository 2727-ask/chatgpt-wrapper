...
async function queryAi(message, context) {
    ...
    let answer;
    if (isCopyButtonPresent === true) {
        answer = getAnswer()
    }
    return answer;
}

async function queryAiCode(message, context) {
    let queryString, innerHTML;
    if (context === "") {
        queryString = message + "\n";
    } else {
...
async function waitForNewButton(page, selector) {
    let initialCount;

    // Get the initial count of elements matching the selector
    try {
        initialCount = await page.$$eval(selector, elements => elements.length);
        console.log(`Initial count of buttons: ${initialCount}`);
    } catch (error) {
        console.error("Error getting initial count of buttons:", error);
        initialCount = 0; // If error, assume no buttons initially
    }
    ...
    try {
        ...
        console.log("A new button with 'Bad response' aria-label appeared on the UI.");
        return true;
    } catch (error) {
        console.error("No new button appeared within the timeout period.");
        return false;
    }
}


async function waitForCopyButtonToAppear() {
    try {
        const buttonSelector = 'button[aria-label="Bad response"]';
...
async function getAnswer() {
    let innerHTML = "";
    let _answerSelector = "div .markdown";
    innerHTML += await getInnerHtmlOfLastElem(_answerSelector);
    return innerHTML;
}
...
module.exports = {
    startBrowser, visitPage, closeBrowser, type, selectElem,
    selectElemWithIndex, writeInTextArea, getInnerHtmlOfLastElem,
    queryAi, retry, saveCookies, newChat, loadOlderChats, getGptList, getChatList, goToChat, getAnswer
};
